...
def adam(w, dw, config=None):
  ...
  pass
  config['t'] += 1
  v_new = config['beta1']*config['v'] + (1-config['beta1'])*dw
  a_new = config['beta2']*config['a'] + (1-config['beta2'])*np.multiply(dw, dw)
  v = v_new/(1 - config['beta1']**config['t'])
  a = a_new/(1 - config['beta2']**config['t'])
