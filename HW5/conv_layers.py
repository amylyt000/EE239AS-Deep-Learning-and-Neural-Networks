...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for n in np.arange(N):
    for f in np.arange(F):
      for h_pr in np.arange(H_prime):
        for w_pr in np.arange(W_prime):
          out[n, f, h_pr, w_pr] = np.sum(x_pad[n,:,(h_pr*stride):(h_pr*stride+HH),\
                                         (w_pr*stride):(w_pr*stride+WW)]*w[f,:,:,:]) + b[f]
...
def conv_backward_naive(dout, cache):
  ...
  for i in np.arange(N):
      for j in np.arange(F):
          ...
          for k in np.arange(H_prime):
              for l in np.arange(W_prime):
                  dw[j] += xpad[i, :, (k*stride):(k*stride+HH), (l*stride):\
                                                                (l*stride+WW)]*dout[i,j,k,l]
...
def max_pool_forward_naive(x, pool_param):
  ...
  H_prime = (H - stride) // stride + 1
  W_prime = (W - stride) // stride + 1
