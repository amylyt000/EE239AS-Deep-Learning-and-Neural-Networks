...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (x, out, beta, gamma, x_hat, mu, var, eps)
    ...
    x_hat = (x - mu)/np.sqrt(var+eps)
    out = gamma*x_hat+beta
    cache = (mu, var, x_hat, out, gamma, beta)
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
  x, out, beta, gamma, x_hat, mu, var, eps = cache
  N, D = x_hat.shape
  ...
  db = (x-mu) * dx_hat
  dc = -db/(var+eps)
  de = -0.5/(np.sqrt(var+eps)**3)*db
  dvar = np.sum(de, axis = 0)
  dx = dx_hat/np.sqrt(var+eps) + 2*(x-mu)*dvar/N + dmu/N
